...
Sub Insert_Record (Src As Object, Args As FormViewInsertedEventArgs)
    ...
    Sub EditGrid_SelectedIndexChanged1(ByVal sender As Object, ByVal Args As EventArgs) Handles EditGrid.SelectedIndexChanged1
   ...
   "Data Source=" & Server.MapPath("time.mdb"))
        DBConnection.Open()
        SQLString = "Select * from [time] Where LastName = '" & Name.SelectedValue & "' order by UseDate DESC"
        DBCommand = New OleDbCommand(SQLString, DBConnection)
        EditGrid.DataBind()
...
<InsertRowStyle BackColor="#00EE00"></InsertRowStyle>
  ...
  SelectCommand = "SELECT ID, UseDate, ClockIn, ClockOut, TotalTime, OutReason, Location, LastName FROM [Time] where ([LastName] = ?) ORDER BY LastName, ID ASC">
  <SelectParameters>
